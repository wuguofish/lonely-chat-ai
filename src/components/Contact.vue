    ...
    <section id="contact" class="py-24 scroll-mt-24">
        <div class="container mx-auto px-4">
            ...
            <div class="mt-10 flex flex-col md:flex-row items-center gap-10">
                <img src="/profile.jpg" alt="阿童的自畫像" class="rounded-full w-48 h-48 object-cover shadow-lg flex-none" />
                <div class="flex-1">
                ...
                <a href="https://portaly.cc/atone0331/support" target="_blank" rel="noopener"
                    class="flex-none font-cubic text-lg border-2 border-berry text-berry rounded-full px-6 py-2 hover:bg-berry hover:text-paper transition-colors">
                    ♡ 請阿童喝手搖
                </a>
            </div>
                ...
                <ul class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 flex-1 font-cubic text-lg">
                    <li v-for="place in places" :key="place.url">
                        <a :href="place.url" target="_blank" rel="noopener"
                            class="flex h-full flex-col items-center justify-center gap-1 text-center bg-choco text-on-choco rounded-lg px-4 py-3 hover:opacity-90 leading-tight">
                            <svg viewBox="0 0 24 24" class="w-6 h-6 fill-current" aria-hidden="true">
                                <path :d="icons[place.icon]" />
                            </svg>
                            <span>{{ place.name }}</span>
                            <span class="text-sm opacity-80">{{ place.note || '　' }}</span>
                        </a>
                    </li>
                </ul>
            </div>
...
// 圖示路徑來自 Simple Icons（CC0）；PixAI 沒有收錄，用一顆四角星代替
const icons = {
...
const places = [
    { name: "PixAI", icon: "sparkle", url: "https://pixai.art/@atone/artworks?utm_source=eap&utm_medium=social&utm_content=atone0331" },
    { name: "Threads", icon: "threads", url: "https://www.threads.net/@atoneplaygame" },
    { name: "pixiv", icon: "pixiv", url: "https://www.pixiv.net/users/2852069" },
    { name: "GitHub", note: "wuguofish", icon: "github", url: "https://github.com/wuguofish" },
    { name: "GitHub", note: "Tsun-u", icon: "github", url: "https://github.com/Tsun-u" }
];
</script>
